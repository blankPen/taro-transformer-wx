  ...
  handleConditionExpr ({ parentNode, parentPath, statementParent }: JSXHandler, jsxElementPath: NodePath<t.JSXElement>) {
    ...
    if (t.isLogicalExpression(parentNode)) {
      ...
      if (operator === '&&' && t.isExpression(left)) {
        /**
         * 如果负责表达式组合 {this.func() && <View />}
         * 抽离出匿名状态变量
         * 
         * const anonymousState = this.func();
         * {anonymousState && <View />}
         */
      ...
      if (t.isJSXElement(consequent) && this.isLiteralOrUndefined(alternate)) {
        ...
        if (confident && !value || t.isIdentifier({ name: 'undefined' })) {
          /**
           * 处理情况 {test ? <View /> : undefined}
           * 
           * 转换为 {<block wx:if={test}><View /></block>}
           */
          ...
          /**
           * 处理情况 {test ? <View /> : 'string'}
           * 
           * 转换为 {<block><block wx:if={test}><View /></block><block wx:else>{'string'}</block></block>}
           */
        ...
        if (confident && !value || t.isIdentifier({ name: 'undefined' })) {
          ...
          /**
           * 处理情况 {test ? 'string' : <View />}
           * 
           * 转换为 {<block><block wx:if={test}>{'string'}</block><block wx:else><View /></block></block>}
           */
          const block2 = buildBlockElement()
        ...
        /**
         * 处理情况 {test ? <View /> : <View2 />}
         * 
         * 转换为 {<block><block wx:if={test}><View /></block><block wx:else><View2 /></block></block>}
         */
        const block2 = buildBlockElement()
        block.children = [consequent]
        newJSXIfAttr(block, test)
        ...
        /**
         * 处理情况 {test ? <View /> : this.func()}
         * 
         * 转换为 
         * const anonymousState = this.func();
         * {test ? <View /> : anonymousState}
         * 这里之后还会再处理一次
         */
        ...
        /**
         * 处理情况 {test ? this.func() : <View />}
         * 
         * 转换为
         * const anonymousState = test ? this.func() : null;
         * {test ? anonymousState : <View />}
         */
  ...
  private loopComponentVisitor: Visitor = {
    VariableDeclarator: (path) => {
      ...
      if (
        ...
        // 找到从 this 解构 并且结构属性包含state/props;
        // 找到 const { state, props } = this;
        ...
        // 删除解构中的state属性，抽离单独成 const state = this.state;
        /**
         * 感觉这块有些问题
         * const { state, props } = this; 
         * 会变成
              ...
              if (callExpr.isCallExpression()) {
                ...
                if (
                  ...
                  if (t.isCallExpression(ary) || isContainFunction(callExpr.get('callee').get('object'))) {
                    /**
                     * this.xxx.func().map(()=> any);
                     * ---
                     * const $anonymousCallee_id = this.xxx.func();
                     * $anonymousCallee_id.map(()=> any);
                     */
                  ...
                  if (
                    ...
                    /**
                     * 给Array.map的 JSXElement上加上 wx:for的 必要参数
                     * {arr.map(()=> <View />)} => {arr.map(()=> <View wx:for={arr} wx:for-item="_item" />)}
                     * {arr.map((v, i)=> <View />)} => {arr.map(()=> <View wx:for={arr} wx:for-item="v" wx:for-index="i" />)}
                     */
  ...
  private jsxAttrVisitor: Visitor = {
    JSXExpressionContainer: (path) => {
      ...
      if (expression.isStringLiteral()) {
        // <View attr={'string'} /> => <View attr='string' />
            ...
            (args as NodePath<t.Node>[]).forEach((arg, index) => {
              ...
              if (index === 0) {
                ...
                if (t.isIdentifier(node) && path.scope.hasBinding(argName)) {
                  ...
                  // 处理 xxx={this.xxx.bind(this, func())}
